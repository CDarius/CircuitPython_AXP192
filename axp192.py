...
class AXP192():
    ...
    **Quickstart: Importing and using the device**
    ...
    Now you can get the AXP192 status
    ...
    def battery_charging_enabled(self, enabled: bool) -> None:
        if isinstance(enabled, bool):
            if enabled:
                self._set_bit_in_register(_AXP192_CHARGING_CTRL1, _AXP192_CHARGING_CTRL1_ENABLE)
            else:
                self._clear_bit_in_register(_AXP192_CHARGING_CTRL1, _AXP192_CHARGING_CTRL1_ENABLE)
        else:
    ...
    def all_adc_enabled(self, enable: bool) -> None:
        self._write_register8(AXP192_ADC_ENABLE_1, 0xFF if enable else 0x00)
    ...
    def power_key_was_pressed(self) -> (bool, bool):
        ...
        if short_press or long_press:
            reg_val = _AXP192_IRQ_3_STATUS_PEK_SHORT_PRESS | _AXP192_IRQ_3_STATUS_PEK_LONG_PRESS
    ...
    def power_off(self) -> None:
        ...
        self._set_bit_in_register(_AXP192_POWER_OFF_BATT_CHGLED_CTRL, _AXP192_POWER_OFF_BATT_CHGLED_CTRL_OFF)
    ...
    def _exten(self, value: bool) -> None:
        ...
        if value:
            self._set_bit_in_register(_AXP192_DCDC13_LDO23_CTRL, _AXP192_DCDC13_LDO23_CTRL_EXTEN)
        else:
            self._clear_bit_in_register(_AXP192_DCDC13_LDO23_CTRL, _AXP192_DCDC13_LDO23_CTRL_EXTEN)
    ...
    def _backup_battery_charging_enable(self) -> bool:
        ...
        return (self._read_register8(_AXP192_BACKUP_BATT) & _AXP192_BACKUP_BATT_CHARGING_ENABLE) != 0
        
    @_backup_battery_charging_enable.setter
    def _backup_battery_charging_enable(self, value: bool) -> None:
        if isinstance(value, bool):
            if value:
                self._set_bit_in_register(_AXP192_BACKUP_BATT, _AXP192_BACKUP_BATT_CHARGING_ENABLE)
            else:
                self._clear_bit_in_register(_AXP192_BACKUP_BATT, _AXP192_BACKUP_BATT_CHARGING_ENABLE)
        else:
            raise ValueError("value must be a boolean True or False")
    ...
    def __read_ldo23_setpoint(self, num: int) -> float:
        ...
        reg_val = (self._read_register8(_AXP192_LDO23_OUT_VOLTAGE) >> value_offset) & 0xF
    ...
    def _set_gpio_ldo_voltage_out(self, gpio_num: int, voltage: float) -> None:
        ...
        if gpio_num == 0:
            if isinstance(voltage, (int, float)):
                ...
                raise ValueError(f"Voltage must be a number")
        else:
    ...
    def _set_gpio_pwm_out(self, gpio_num: int, duty_cycle: int) -> None:
        ...
        if 1 <= gpio_num <= 2:
            if isinstance(duty_cycle, int):
                if 0 <= duty_cycle <= 255:
                    pwm_reg = _AXP192_PWM1_DUTY_RATIO_Y1 if gpio_num == 1 else _AXP192_PWM2_DUTY_RATIO_Y1
                    self._write_register8(pwm_reg, 0xFF)
                    self._write_register8(pwm_reg + 1, duty_cycle)
                    self.__set_gpio_function(gpio_num, 0x02)
                else:
                    raise ValueError(f"Duty cycle out of range. Allowed range 0-255")
            else:
                raise ValueError(f"Duty cycle must be an integer number from 0 to 255")
        else:
